...
- `apps/worker`: VC単位のセッション管理APIを提供するCloudflare Workerです。セッション状態はDurable Objects、録音データはR2に保存します。
- `services/voice-gateway`: Discord Voice への接続と録音を担うNode.jsサービスです。録音完了後にWorker API経由で状態を更新し、R2へアップロードします。
